...
/**
 * Emission Factor Version Airtable Service
 * 
 * Handles all Airtable API interactions for Emission Factor Version table.
 * This service can be replaced with a PostgreSQL service later
 * without changing the repository interface.
 */
export class EmissionFactorVersionAirtableService {
  ...
  constructor() {
    ...
    this.tableName = process.env.AIRTABLE_EMISSION_FACTOR_VERSION_TABLE_ID || 
                     process.env.AIRTABLE_EMISSION_FACTOR_VERSION_TABLE_NAME || 
                     'Emission Factor Version' // Default table name in Airtable
    
    console.log(`🌿 EmissionFactorVersionAirtableService initialized:`)
    ...
    try {
      ...
      if (fs.existsSync(this.cacheFilePath)) {
        ...
        if (age < 60 * 60 * 1000) {
          ...
          console.log(`📊 Loaded Emission Factor Version total count from disk cache: ${cacheData.count}`)
        }
      }
    } catch (error) {
      console.warn('⚠️  Could not load Emission Factor Version count cache from disk:', error)
    }
  }
  ...
  private saveCacheToDisk(): void {
    try {
      ...
      console.warn('⚠️  Could not save Emission Factor Version count cache to disk:', error)
    }
  }
  ...
  private clearCountCache(): void {
    ...
    try {
      ...
      console.warn('⚠️  Could not delete Emission Factor Version count cache file:', error)
    }
  }
  ...
  /**
   * Map Airtable record to EmissionFactorVersion interface
   */
  mapAirtableToEmissionFactorVersion(record: Airtable.Record<any>): any {
    ...
    // Map Airtable field names to our EmissionFactorVersion interface
    // Emission Factor Version table fields: Name, Short code, Description, Formula, Category, Status, Notes
  ...
  /**
   * Map EmissionFactorVersion DTO to Airtable fields
   */
  mapEmissionFactorVersionToAirtable(dto: any): Record<string, any> {
  ...
  /**
   * Get all Emission Factor Version records from Airtable
   */
  async findAll(): Promise<any[]> {
    try {
      console.log(`📥 Fetching ALL Emission Factor Version records from Airtable table: ${this.tableName}`)
      ...
      return records.map(record => this.mapAirtableToEmissionFactorVersion(record))
    } catch (error: any) {
      console.error('Error fetching Emission Factor Version from Airtable:', error)
      ...
      throw new Error(`Failed to fetch Emission Factor Version from Airtable: ${error.error || error.message || 'Unknown error'}`)
    }
  }

  /**
   * Get paginated Emission Factor Version records from Airtable
   */
  async findPaginated(
    offset: number = 0, 
    limit: number = 50, 
    ...
    try {
      console.log(`📥 Fetching paginated Emission Factor Version: offset=${offset}, limit=${limit}`)
      ...
      return {
        records: paginatedRecords.map(record => this.mapAirtableToEmissionFactorVersion(record)),
        total,
      }
    } catch (error: any) {
      console.error('❌ Error fetching paginated Emission Factor Version from Airtable:', error)
      ...
      throw new Error(`Failed to fetch Emission Factor Version: ${error.error || error.message || 'Unknown error'}`)
    }
  }
  ...
  async findById(id: string): Promise<any | null> {
    try {
      ...
      return this.mapAirtableToEmissionFactorVersion(record)
    } catch (error: any) {
  ...
  async create(fields: Record<string, any>): Promise<any> {
    try {
      console.log(`📝 Creating Emission Factor Version in Airtable`)
      const airtableFields = this.mapEmissionFactorVersionToAirtable(fields)
      ...
      console.log(`✅ Successfully created Emission Factor Version in Airtable: ${record.id}`)
      ...
      console.error('❌ Error creating Emission Factor Version in Airtable:', error)
      throw new Error(`Failed to create Emission Factor Version: ${error.error || error.message || 'Unknown error'}`)
    }
  }

  /**
   * Update a Emission Factor Version record
   */
  async update(id: string, fields: Record<string, any>): Promise<any> {
    try {
      console.log(`📝 Updating Emission Factor Version in Airtable: ${id}`)
      const airtableFields = this.mapEmissionFactorVersionToAirtable(fields)
      
      const record = await this.base(this.tableName).update(id, airtableFields)
      console.log(`✅ Successfully updated Emission Factor Version: ${id}`)
      
      return this.mapAirtableToEmissionFactorVersion(record)
    } catch (error: any) {
      console.error(`❌ Error updating Emission Factor Version ${id}:`, error)
      if (error.statusCode === 404 || error.error === 'NOT_FOUND') {
        return null
      }
      throw new Error(`Failed to update Emission Factor Version: ${error.error || error.message || 'Unknown error'}`)
    }
  }
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      throw new Error(`Failed to delete Emission Factor Version: ${error.error || error.message || 'Unknown error'}`)
    }
  }
  ...
  async getTotalCount(lazy: boolean = true): Promise<number> {
    try {
      if (this.totalCountCache && Date.now() - this.totalCountCache.timestamp < this.COUNT_CACHE_TTL) {
        console.log(`📊 Using cached total count for Emission Factor Version: ${this.totalCountCache.count}`)
        return this.totalCountCache.count
      }

      if (lazy && !this.totalCountCache) {
        console.log(`📊 No cache available for Emission Factor Version, returning estimate and calculating in background...`)
        const estimate = 100
      ...
      console.error('Error getting Emission Factor Version total count:', error)
      if (this.totalCountCache) {
  ...
  private calculateTotalCountInBackground(): void {
    ...
    this.countCalculationPromise = this.calculateTotalCount()
      ...
      .catch(error => {
        console.error('Error calculating Emission Factor Version total count in background:', error)
        this.countCalculationPromise = null
      })
  }
  ...
  private async calculateTotalCount(): Promise<number> {
    try {
      ...
      console.error('Error calculating Emission Factor Version total count:', error)
      return 0
    }
  }
  ...
  private async getFilteredCount(filterFormula: string): Promise<number> {
    try {
      ...
      console.error('Error getting filtered count for Emission Factor Version:', error)
      return 0
    }
  }
  ...
  async getDistinctValues(fieldName: string, limit: number = 1000): Promise<string[]> {
    try {
      ...
      if (cached && Date.now() - cached.timestamp < this.DISTINCT_VALUES_CACHE_TTL) {
        console.log(`📊 Using cached distinct values for Emission Factor Version ${fieldName}: ${cached.values.length} values`)
        return cached.values
      }
      
      console.log(`📊 Fetching distinct values for Emission Factor Version field: ${fieldName}`)
      ...
      console.log(`✅ Found ${values.length} distinct values for Emission Factor Version ${fieldName}`)
      ...
      console.error(`❌ Error getting distinct values for Emission Factor Version ${fieldName}:`, error)
      return []
    }
  }
...
let emissionFactorVersionAirtableServiceInstance: EmissionFactorVersionAirtableService | null = null

export const getEmissionFactorVersionAirtableService = (): EmissionFactorVersionAirtableService => {
  if (!emissionFactorVersionAirtableServiceInstance) {
    emissionFactorVersionAirtableServiceInstance = new EmissionFactorVersionAirtableService()
  }
  return emissionFactorVersionAirtableServiceInstance
}
