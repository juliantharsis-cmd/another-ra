...
import { applicationListConfig } from '@/components/templates/configs/applicationListConfig'
import { isFeatureEnabled } from '@/lib/featureFlags'

/**
 * Application List Page using ListDetailTemplate
 * 
 * This page displays the Application List table from Airtable
...
export default function ApplicationListPage() {
  ...
  const isApplicationListsEnabled = isFeatureEnabled('applicationLists')
  
  // If feature flag is disabled, show a message
  if (!isApplicationListsEnabled) {
    return (
        ...
        <div className={`flex-1 p-8 overflow-hidden flex flex-col ${isCollapsed ? 'ml-16' : 'ml-64'}`}
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <h2 className="text-2xl font-semibold text-neutral-700 mb-2">Application List Feature</h2>
  ...
  return (
    <div className="fixed inset-0 flex bg-gray-50 overflow-hidden" style={{ margin: 0, padding: 0 }}>
      ...
      <div className={`flex-1 p-8 overflow-hidden flex flex-col ${isCollapsed ? 'ml-16' : 'ml-64'}`}
           style={{ transition: 'margin-left 300ms ease-in-out' }}>
        <ListDetailTemplate config={applicationListConfig} />
      </div>
    </div>
  )
}
