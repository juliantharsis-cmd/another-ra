...
import { ghgTypeConfig } from '@/components/templates/configs/ghgTypeConfig'
import { isFeatureEnabled } from '@/lib/featureFlags'

/**
 * GHG Type Page using ListDetailTemplate
 * 
 * This page displays the GHG Type table from Airtable
 * using the reusable ListDetailTemplate component.
 * Matches the exact structure and styling of the Geography and EF GWP pages.
 */
export default function GHGTypePage() {
  const { isCollapsed } = useSidebar()
  const isGHGTypesEnabled = isFeatureEnabled('ghgTypes')
  
  // If feature flag is disabled, show a message
  if (!isGHGTypesEnabled) {
    return (
        ...
        <div className={`flex-1 p-8 overflow-hidden flex flex-col ${isCollapsed ? 'ml-16' : 'ml-64'}`}
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <h2 className="text-2xl font-semibold text-neutral-700 mb-2">GHG Type Feature</h2>
  ...
  return (
    <div className="fixed inset-0 flex bg-gray-50 overflow-hidden" style={{ margin: 0, padding: 0 }}>
      ...
      <div className={`flex-1 p-8 overflow-hidden flex flex-col ${isCollapsed ? 'ml-16' : 'ml-64'}`}
           style={{ transition: 'margin-left 300ms ease-in-out' }}>
        <ListDetailTemplate config={ghgTypeConfig} />
      </div>
    </div>
  )
}
