 ...
 * 
 * @param existingFilters - Manually configured filters
 * @param autoGeneratedFilters - Auto-generated filters
 * @returns Merged array of filters
 */
export function mergeFilters(
  existingFilters: FilterConfig[] = [],
  autoGeneratedFilters: FilterConfig[] = []
): FilterConfig[] {
  const existingKeys = new Set(existingFilters.map(f => f.key.toLowerCase()))
  
  // Filter out auto-generated filters that conflict with existing ones
  const uniqueAutoFilters = autoGeneratedFilters.filter(
    f => !existingKeys.has(f.key.toLowerCase())
  )
  
  // Return existing filters first, then auto-generated ones
  return [...existingFilters, ...uniqueAutoFilters]
}
